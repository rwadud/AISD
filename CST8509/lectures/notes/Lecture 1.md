...
**Q-learning**: The process of learning the **action value function** $Q(s, a)$, specifically the **optimal** action value function $Q^{\ast}(s, a)$.

- $Q^{\ast}(s, a)$ is the value function when the agent follows the **optimal policy** $\pi^{\ast}$.
- The star ($*$) means optimized: the value function is as good as it can get.
- The Q value depends on what policy you are following. Under the optimal policy, you get $Q^{\ast}$.
...
| **Q-learning** | An algorithm for learning the optimal action value function $Q^{\ast}$ |
