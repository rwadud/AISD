...
> Math is rendered with GitHub's LaTeX support: inline `$...$` and block `$$...$$`.
...
**Example:** 4 of 5 claims supported → $\dfrac{4}{5} = \mathbf{0.8}$.

### 1.6 RAGAS — Context Precision

$$\text{Precision} = \frac{\text{relevant sentences}}{\text{total retrieved sentences}}$$

**Example:** 1 relevant of 10 retrieved → $\dfrac{1}{10} = \mathbf{0.1}$ (noisy retrieval).

### 1.7 RAGAS — Answer Relevance

$$\text{Relevance} = \frac{1}{N} \sum_{i=1}^{N} \operatorname{sim}(q_i^{\text{gen}},\ q^{\text{orig}})$$

Generate $N$ candidate questions from the answer; average cosine similarity to the original question.
...
**Example:** $\mu=100,\ \sigma=15,\ X=130 \Rightarrow Z = \dfrac{130-100}{15} = \mathbf{2.0}$ (2 σ above mean).

### 2.3 Box-Cox Transformation

$$X' = \begin{cases} \dfrac{X^{\lambda} - 1}{\lambda} & \text{if } \lambda \neq 0 \\[6pt] \ln(X) & \text{if } \lambda = 0 \end{cases}$$

**Example:** $\lambda = 0.5,\ X = 16 \Rightarrow \dfrac{16^{0.5} - 1}{0.5} = \dfrac{4 - 1}{0.5} = \mathbf{6}$.
...
- $\cos(\theta) = \dfrac{28}{3.742 \cdot 7.483} \approx \mathbf{1.0}$ (same direction)
...
$$\text{Avg} = \left[\tfrac{1+3+5}{3}, \tfrac{2+4+0}{3}\right] = \mathbf{[3,\ 2]} \qquad \text{Max} = \mathbf{[5,\ 4]}$$
...
$$\rho(\tau) = \operatorname{Corr}(X_t,\ X_{t+\tau})$$
...
$$\text{effective\_batch} = \text{batch\_size} \times \text{accumulation\_steps}$$

**Example:** $4 \times 4 = \mathbf{16}$.

### 5.3 Batches per Epoch

$$\text{batches} = \left\lceil \frac{N_{\text{samples}}}{\text{batch\_size}} \right\rceil$$
...
$$\text{memory (bytes)} = N_{\text{params}} \times \text{bytes/param}$$
...
$$W' = W + \Delta W, \quad \Delta W = A \cdot B, \quad A \in \mathbb{R}^{m\times k},\ B \in \mathbb{R}^{k\times n},\ k \ll \min(m,n)$$
...
$$L_{\text{soft}} = T^2 \cdot \operatorname{KL}\!\left(\operatorname{softmax}(z_T/T)\ \|\ \operatorname{softmax}(z_S/T)\right)$$

$$L_{\text{hard}} = \operatorname{CE}(z_S,\ y)$$
...
| 12 | LoRA | $W + A\cdot B$ |
| 13 | Distill loss | $\alpha\, T^2\,\operatorname{KL} + (1-\alpha)\,\operatorname{CE}$ |
| 14 | G-Eval | $\sum s\cdot P(s)$ |
| 15 | Faithfulness | $\dfrac{\text{supported}}{\text{total}}$ |
| 16 | Grid cost | $O(v^k)$ |
| 17 | DARTS mix | $\sum_i \operatorname{softmax}(\alpha_i)\,o_i(x)$ |
...
$$\text{requests/day} = \text{throughput (req/s)} \times 86{,}400\ \text{s}$$
...
$$\text{latency}_{\text{eff}} = p_{\text{hit}} \cdot t_{\text{cache}} + (1 - p_{\text{hit}}) \cdot t_{\text{model}}$$

**Example:** $t_{\text{model}}=10$ ms, $t_{\text{cache}}=0.1$ ms, $p_{\text{hit}}=0.6$

$$\text{latency}_{\text{eff}} = 0.6(0.1) + 0.4(10) = \mathbf{4.06}\ \text{ms}\ (\approx 2.5\times \text{throughput})$$
